        ...
        <div className="modal">
            <div className="modalContent">
                ...
                <div className="modalBody">
                    <p>Registration Code: <i>{registrationCode}</i></p>
                    <p>Last Use Date: <i>{modalData.lastUseTime}</i></p>
                    <input type='date' onChange={(e) => setLastUseTime(e.target.value)} value={lastUseTime}></input>
                    <p>Total Distance Traveled: <i>{modalData.distance}</i> Km.</p>
                    <p>Todays Distance: </p>
                    <input type="number" onChange={(e) => setDistance(e.target.value)} 
                    value={distance}
                    placeholder='km' />
                    <p>Open for ride?</p>
                    <b>Yes <input type="checkbox" onChange={e => setIsBusy(e ? 1 : 0) } checked={isBusy ? 1 : 0} /></b>
                    <b>No <input type="checkbox" onChange={e => setIsBusy(e ? 0 : 1) } checked={isBusy ? 0 : 1} /></b>
